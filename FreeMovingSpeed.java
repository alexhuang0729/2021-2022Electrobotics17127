/*
 *Team 17127 Electrobotics, based in Eagan, MN. Changed 01/08/2022, at 10:30 AM.
 */
...
public class FreeMovingSpeed extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            double Turboboost = gamepad1.left_trigger;
            ...
            double FlipHalf = gamepad2.right_trigger;
            flipPower    = (Range.clip(flip, -1.0, 1.0))*.5 ;
            if (slowDown<.10) {
                if (Turboboost > 0.05){
                    rightDrive1.setPower(-1/*(-pivot + (vertical - horizontal))*(4/3)*/);
                    rightDrive2.setPower(-1/*(-pivot + (vertical + horizontal))*(4/3)*/);
                    leftDrive1.setPower(-1/*(pivot + (vertical + horizontal))*(4/3)*/);
                    leftDrive2.setPower(-1/*(pivot + (vertical - horizontal))*(4/3)*/);
                }
                else {
            ...
            if (duckSpinnerRightBumper){
                DuckMotor.setVelocity(200);//Should be negative when on the red side; positive when on the blue side
                //DuckMotor.setPower(duckPower);
            }
            else if (duckSpinnerLeftBumper){
                DuckMotor.setVelocity(-190);//Should be negative when on the red side; positive when on the blue side
                //DuckMotor.setPower(-duckPower);
            }
            else{
